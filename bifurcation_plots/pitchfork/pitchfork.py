...
class Pitchfork():
    ...
    def plot_poly(self, x_mu, idx_poly, init_poly=False,transparent=True):
        fig, ax = plt.subplots(1,1, figsize=(10.5,5))
        # plot the bifurcation diagram
        idx_0 = np.argmax(x_mu>=0)
        ax.plot(x_mu, [0 for _ in x_mu],color="red",linestyle=":", label="solution branches")
        ...
        fig.savefig(f"poly_{idx_poly}.pdf", bbox_inches="tight", transparent=transparent)
        plt.close(fig)

pitchfork = Pitchfork( mu=cp.Uniform(0.8,1.2), 
                    base_mu=cp.Uniform(-np.sqrt(3),np.sqrt(3)), 
                    n_samples=1000)

pitchfork.run(degree_pc=5, 
                n_init=15)

pitchfork.plot_poly(x_mu = np.linspace(-1,2,2000), 
                    idx_poly=[0,1,2,3], 
                    init_poly=False, 
                    transparent=False)
