...
class SphericalSystem():
    ...
    def plot_xy_mu(self):
        ...
        fig.tight_layout()
        fig.savefig(f"plots/SphericalSystem2_2d_xy_rho_N_{degree_pc}_rho_{self.mu}_N_init_{len(valid_branches)}.pdf", bbox_inches='tight')
    ...
    def plot_3d_bifurcation(self):
        ...
        fig.savefig(f"plots/SphericalSystem2_3d_xy_rho_N_{degree_pc}_rho_{self.mu}_N_init_{len(valid_branches)}.pdf", bbox_inches='tight', pad_inches=0.1)
        plt.show()

if __name__ == "__main__":
    degree_pc = 10
    mu_values=[cp.Uniform(-0.1, 0.1), cp.Uniform(-0.3, 0.3), cp.Uniform(-1, 1)]
    std=[0.1, 0.3, 1]
    n_branch=[20, 20, 20]
